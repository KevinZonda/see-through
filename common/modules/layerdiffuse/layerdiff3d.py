from diffusers.models.unets.unet_3d_blocks import CrossAttnDownBlockSpatioTemporal, DownBlockSpatioTemporal, UpBlockSpatioTemporal, CrossAttnUpBlockSpatioTemporal
from diffusers.models.unets.unet_2d_blocks import CrossAttnDownBlock2D, DownBlock2D, UNetMidBlock2DCrossAttn, CrossAttnUpBlock2D
from diffusers import UNetSpatioTemporalConditionModel, UNet2DConditionModel
...
from diffusers.models.unets.unet_2d_blocks import (
    get_down_block as get_down_block_unet2d,
    get_mid_block as get_mid_block_unet2d,
    get_up_block as get_up_block_unet2d,
    ResnetBlock2D,
    DownBlock2D,
    UpBlock2D,
    UNetMidBlock2D,
...
class CrossFrameAttnUpBlock(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class CrossFrameAttnDownBlock(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
def get_down_block(
    ...
    **kwargs
    ...
    if down_block_type == "CrossFrameAttnDownBlock":
        return CrossFrameAttnDownBlock(**kwargs)
    else:
        return get_down_block_unet2d(down_block_type, **kwargs)
...
def get_mid_block(
    ...
    **kwargs
    ...
    if mid_block_type == "UNetMidBlockCrossFrameAttn":
        return UNetMidBlockCrossFrameAttn(**kwargs)
    else:
        return get_mid_block_unet2d(mid_block_type, **kwargs)
...
def get_up_block(
    ...
    **kwargs
    ...
    if up_block_type == "CrossFrameAttnUpBlock":
        ...
        return get_up_block_unet2d(up_block_type, **kwargs)


class GroupEmbedding(nn.Module):
    def __init__(self, n_cls, ndim, *args, **kwargs):
...
@dataclass
class UNetFrameConditionOutput(BaseOutput):
    """
...
class UNetFrameConditionModel(
    ...
    def __init__(
        ...
        group_embedding_num: Tuple[int] = (13, 11),
        tag_version="v2"
    ):
        super().__init__()
        ...
        if group_embedding:
            ...
            self.group_embeds2 = nn.ModuleList()
            # torch.Size([11, 1280]) torch.Size([11, 77, 2048]) tensor([1])
            for n in group_embedding_num:
                self.group_embeds.append(GroupEmbedding(n, 1280))
                self.group_embeds2.append(GroupEmbedding(n, 2048))
        else:
            self.group_embeds = None
    ...
    def init_weights(self):
        for name, module in self.named_modules():
            if '.temporal_transformer_blocks.' in name and '.ff.net.2' in name:
                zero_module(module)
        if self.config.group_embedding:
            for e in self.group_embeds:
                e.linear = zero_module(e.linear)
            for e in self.group_embeds2:
                e.linear = zero_module(e.linear)
    ...
    def forward(
        ...
        timestep = timestep.repeat_interleave(num_frames)
        sample = sample.reshape(batch_size * num_frames, *sample.shape[2:])
        # encoder_hidden_states = encoder_hidden_states.reshape(batch_size * num_frames, *encoder_hidden_states.shape[2:])
        ...
        if self.config.group_embedding:
            aug_layer = self.group_embeds[group_index]
        else:
            aug_layer = None
        ...
        for downsample_block in self.down_blocks:
            if isinstance(downsample_block, CrossFrameAttnDownBlock):
                frame_kwargs = {'num_frames': num_frames}
            else:
                frame_kwargs = {}
            ...
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                sample, res_samples = downsample_block(hidden_states=sample, temb=emb, **frame_kwargs)
                if is_adapter and len(down_intrablock_additional_residuals) > 0:
        ...
        if self.mid_block is not None:
            if isinstance(self.mid_block, UNetMidBlockCrossFrameAttn):
                frame_kwargs = {'num_frames': num_frames}
            else:
                frame_kwargs = {}
            ...
            if hasattr(self.mid_block, "has_cross_attention") and self.mid_block.has_cross_attention:
                ...
                sample = self.mid_block(sample, emb, **frame_kwargs)
        ...
        # 5. up
        for i, upsample_block in enumerate(self.up_blocks):
            if isinstance(upsample_block, CrossFrameAttnUpBlock):
                frame_kwargs = {'num_frames': num_frames}
            else:
                frame_kwargs = {}
        ...
        return UNetFrameConditionOutput(sample=sample)
